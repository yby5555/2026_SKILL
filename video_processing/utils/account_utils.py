...
account_utils.py
===================
Flow 账号状态检测器（迁移到 video_processing/utils/）
...
用法（独立运行，检测单个账号）：
    python account_utils.py email@gmail.com
...
# ── 路径修复 ──────────────────────────────────────────────────────────────────
_VIDEO_PROCESSING = Path(__file__).resolve().parent.parent.parent   # 2026_SKILL/
_ACCOUNT_MGR = _VIDEO_PROCESSING / "account_mgr"

for _p in [str(_VIDEO_PROCESSING), str(_ACCOUNT_MGR)]:
    if _p not in sys.path:
        sys.path.insert(0, _p)
# ──────────────────────────────────────────────────────────────────────────────

from account_mgr.mongo_utils import (              # noqa: E402
    create_mongo_client, get_collection,
    mark_pending, mark_abnormal,
)
from account_mgr.redis_utils import remove_from_pool  # noqa: E402
